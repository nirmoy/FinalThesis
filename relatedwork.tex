...
%AP: Overuse of hyphen (-). Please do not hyphenate everything.
   ...
   %Nirmoy:add images to describe Hadoop cluster and MapReduce 
	...
	\item NfSen 
       \end{enumerate}
      \emph{nfcapd} reads data from the network and stores it into disk. It also rotate the file to limit size of file.
      \emph{nfdump} allows $tcpdump$ style filter expression for processing NetFlow data stored by \emph{nfcapd}. NfSen gives 
      graphical overview of NetFlow data using RRDTool\cite{rrd};
        \begin{figure}[htb]
          \centering
